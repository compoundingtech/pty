...
A persistent terminal session manager. Run long-lived processes, detach, reconnect later — from any machine over SSH. The npm package name is `@compoundingtech/pty`; the CLI command remains `pty`.
...
Tests use **vitest** and live in `tests/`. The `@compoundingtech/pty/testing` library (`src/testing/`) provides a `Session` class used by the tests — see [docs/testing.md](docs/testing.md) for the full API.
...
src/
  ...
  testing/        Testing library (exported as @compoundingtech/pty/testing)
