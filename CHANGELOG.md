...
- **New event types exported from `@myobie/pty/client`:** `UserEvent`, `StateSetEvent`, `StateDeleteEvent`, plus helpers `emitUserEvent`, `appendEvent`, `isUserEvent`, `validateUserEventType`. `EventBase.type` widened from the `EventType` enum to `string` so subtypes carry their own literals (`user.${string}` / `"state.set"` / `"state.delete"`); existing consumers that switch on the enum values are unaffected.
